...
#   # n_clones = np.ones(max(x)+1, dtype=np.int64) * num_clones
#   container = TableContainer()
#   chmm = CHMM_LCM(n_clones=n_clones, pseudocount=2e-3, x=x, a=a, container=container,alpha=alpha,seed=seed)  # Initialize the model
#   progression = chmm.learn_em_T(x, a, n_iter=niter,
#                                 # term_early=False,
#                                 )  # Training   use n_iter=1000 for better training
#   # refine learning
#   chmm.pseudocount = 0.0
