...
class Environment:
    ...
    # def create_pos_to_state(self, rows, cols):
    #     """
    #     Returns a dictionary mapping (row, col) -> state_id,
    #     where state_id starts at 1 and increments in row-major order.
    #     """
    #     # pos_to_state = {}
    #     state_id = 1
    #     for r in range(rows):
    #         for c in range(cols):
    #             self.pos_to_state[(r, c)] = state_id
    #             state_id += 1
    ...
    def plot_graph(self, T, niter, 
        ...
        for s in range(n_state):
            # Skip clones that have been sanitized (not in clone_dict)
            if s >= self.num_unique_states and s not in self.clone_dict:
                continue
            G.add_node(s)
...
class GridEnvRightDownExtinction(Environment):
    """
    A version of GridEnvRightDownNoSelf where there is NO reward at the terminal state.
    ...
    def step(self, action):
        """
        Step with a guaranteed valid action.
        """
        if action not in self.get_valid_actions():
            raise ValueError(f"Action {action} is not valid from state {self.current_state}.")

        # If we're already in a terminal, episode is over.
        if self.current_state in self.rewarded_terminals + self.unrewarded_terminals:
            return self.current_state, 0, True
...
class GridLatentInhibition(Environment):
    """
    A 3x3 grid with states numbered 1..9:
    ...
    def step(self, action, phase):
        ...
        phase variable determines if the environment is preexposing or conditioning 
        """
        if action not in self.get_valid_actions():
            raise ValueError(f"Action {action} is not valid from state {self.current_state}.")
        ...
        if self.is_terminal(next_state):    
            ...
            if self.visited_cue and phase == 2:
                print("reward")
                reward = +1
            else:
                reward = -1
