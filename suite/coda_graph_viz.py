...
coda_graph_viz.py
=================
Visualization utilities for cognitive graphs from Snapshot objects.

- grid_positions(rows, cols): obs->(x,y) mapping for gridworld (obs are 1..N).
- plot_snapshot(snapshot): draw graph (grid layout with clone jitter OR spring layout).
- save_snapshot_sequence(snapshots, out_dir): save a series of images step-by-step.
...
def grid_positions(rows: int, cols: int, obs_start: int = 1, origin: str = "top-left") -> Dict[Hashable, Tuple[float, float]]:
...
def save_snapshot_sequence(
    snapshots: List[Any],
    out_dir: str,
    *,
    prefix: str = "graph_iter",
    layout: str = "grid",
    obs_pos: Optional[Dict[Hashable, Tuple[float, float]]] = None,
    node_label: str = "sid",
):
    ...
    for i, snap in enumerate(snapshots):
        fig, _ = plot_snapshot(
            snap, title=snap.title, layout=layout, obs_pos=obs_pos, node_label=node_label, show_edge_labels=True
        )
